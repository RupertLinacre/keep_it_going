...
Run `scripts/check-height-browser.js` through Playwright CLI against Vite for actual desktop/phone input, answer feedback, Escape, no-speed-impulse assertions, overflow checks and screenshots. Desktop at 1440×900 and an emulated 390×844 touch phone each accepted eight answers and continued through the opening water jump, at approximately 60 fps with 99th-percentile frame intervals of 16.8 ms and no page errors. Landscape at 844×390 also fitted the viewport. These measurements come from browser contexts on the development Mac, not physical phone hardware.
