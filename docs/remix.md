...
Each effect lasts **20 seconds of active game time**. Pausing freezes the timer. Effects do not stack or replace one another early. Each gate requires **four new correct answers**, including the first. Collecting a gate resets the answer counter; answers during its active effect count towards the next one, capped at four. After expiry there is a three-second breather. Once both conditions are satisfied, the next gate is placed 28–58 route metres ahead, searching for rail with a moderate slope. The first earned gate starts 65 metres ahead. Wrong/partial entries do not count, and extra answers never carry across collections. The shuffled bag includes all seven effects before refilling and avoids identical consecutive effects at bag boundaries.

| Effect | Gameplay | Visual treatment |
| --- | --- | --- |
| Ice glide | Rolling resistance and air resistance fall to 25% of the selected difficulty's usual values. | Icicles grow around the carriage roofs and rims; the ground keeps its world colour. |
...
Races generate the same seeded base course, with separate `HeightTrack` instances for each rider. The six-power bag contains Ice glide, Gravity flip, Cargo carnival, Heavy metal, Tailwind and Sky lift. Only Downhill drift is excluded from races and rejected in packets. Each rider earns and collects gates independently; even the first gate position depends on when that rider completes four answers. Sky lift raises only that rider’s track. Flooded sections share their original pool geometry; raised rails can leave the water.

Protocol 8 includes the current power, age, remaining time, collection count, answer progress and next gate, plus bounded lift states for the raised railway. It carries up to eight boxes per wagon, TNT masks and loose TNT flags, and full 56-droplet bow splashes with spray orientation and strength. Impact IDs are stable per explosion, including water events. Packet and instance-buffer limits cover simultaneous bursts on both tracks. Invalid or disallowed effect data is rejected. The original bounded, smooth opponent playback remains: short rail predictions now use the same power modifiers and water drag as local physics, and loose-object prediction respects reversed/heavy gravity. Local inputs never wait for network delivery.
...
`tests/remix-multiplayer.test.ts` covers seeded geometry with different generation horizons, permitted power bags, race power restrictions, richer packet validation, stable splash IDs and power-aware opponent prediction. Session tests cover mode/seed negotiation and rematches across different entry screens. `scripts/check-remix-multiplayer-browser.js` exercises real WebRTC between desktop Chrome and a touch-phone context, normal answers, the original five independent powers, cargo/TNT replication, two-lane splashes, pause, results, rematches and disconnect recovery.
...
## Answer-earned gates and two-player Sky lift

Ice glide now grows and melts twelve small icicles around each carriage’s roof or open rim. The terrain retains its world colour. Both 3D and Canvas renderers apply the effect to the correct rider only.

Race generation excludes pretzel knots, orients large depth excursions outwards, and blends each exit back to the usual line. Cobra rolls keep a compact footprint in races. Spacing can ease back inward after a wide section leaves the planning window, rather than retaining the widest offset forever.

Sky lift uses an independent copy of the generated rail for each rider. Snapshots carry the lift’s section, extent, start/target height and animation age. Opponent playback applies these on its buffered display clock, and samples the train, couplers, rail meshes, gates and attractions from the same raised track. Rendering replaces provisional section bindings after network setup; local and rival rail buffers are separate. Lift state survives power expiry, frozen water-jump animations and pruning by a racer ahead of the camera. Section generation always uses the unraised base endpoint, preventing double height at new joins. Coach snapshot coordinates use physical follower spacing on stretched connections.

The existing three-times camera limit remains. Nearby racers at different heights share some vertical framing while the local engine stays in view. Four-answer progress is shown in the existing HUD, without an additional mobile overlay.

The complete world race regression also passes across all twelve themed attractions, the original five power effects, two-lane splashes, shared pause, small/rotated phone layouts, matching results, rematches and disconnect recovery, with no browser errors.

Validation for this revision: **161 tests** and the production build pass. `tests/race-power-upgrades.test.ts` covers four-answer gating, race-only generation, return spacing, independent lift snapshots, pruning/new joins, malformed payloads, icicle deployment and smooth lift playback under delayed packets. `scripts/check-race-upgrades-browser.js` joins two actual PeerJS clients, answers through the four-question threshold, checks terrain colour and per-player icicles, and verifies both train positions and rendered rail buffers at unequal heights. It passes with WebGL on both clients and with Canvas on the phone. Five-second lifting samples recorded about 60 FPS with p99 frame intervals at most 17.7 ms and no frame over 50 ms. `scripts/check-moving-lift-race-browser.js` exercises the full moving simulation through a 20-second Sky lift: the riders travelled roughly 980 m and 746 m, remained in play, and retained separate heights with no browser errors. Both measured approximately 60 FPS and 17.7 ms p99. These are Chromium measurements on Apple M4 with phone emulation, not physical phone benchmarks. Local reports and screenshots are in `output/playwright/race-upgrades-validation.json`.
