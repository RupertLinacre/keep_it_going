...
async page => {
 ...
 for(const mobile of [false,true]){
  ...
  const pieces=await page.evaluate(async requested=>{
   ...
   return WORLDS.filter(w=>requested==='all'||w.id===requested).flatMap(w=>w.pieces.filter(kind=>['windmillloop','carouselhelix','midwayloop'].includes(kind)).map(kind=>({world:w.id,kind,start:w.start,end:w.end})));
  },requested);
  for(const piece of pieces){
   const info=await page.evaluate(({world,kind,start,end})=>{
    ...
    g.physics.distance=s.start+8;g.physics.previousDistance=g.physics.distance;g.physics.velocity=28;
    ...
    for(let i=0;i<180;i++){g.elapsed+=1/60;g.physics.distance=s.start+8+i/60*25;g.physics.previousDistance=g.physics.distance;g.carriages.update(1/60,g.physics.distance,25,false);g.draw(ctx)}
    const v=g.view;
    const drive=v.adventureScene.tiles.get(s.id).drives[0];if(drive.speed<1)throw Error('Attraction did not respond');return {driveSpeed:drive.speed,angle:drive.angle,at:s.start,length:s.length,height:Math.max(...s.frames.map(f=>f.position.y)),draws:v.renderer.info.render.calls,triangles:v.renderer.info.render.triangles,
   ...
   await page.screenshot({path:`output/playwright/reactive-${piece.kind}-${mobile?'mobile':'desktop'}.png`});
