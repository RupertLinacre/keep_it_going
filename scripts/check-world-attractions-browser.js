...
async page => {
 ...
 for(const mobile of [false,true]){
  ...
  const pieces=await page.evaluate(async requested=>{
   ...
   return WORLDS.filter(w=>requested==='all'||w.id===requested).flatMap(w=>w.pieces.map(kind=>({world:w.id,kind,start:w.start,end:w.end})));
  },requested);
  for(const piece of pieces){
   const info=await page.evaluate(({world,kind,start,end})=>{
    ...
    const v=g.view;
    return {at:s.start,length:s.length,height:Math.max(...s.frames.map(f=>f.position.y)),draws:v.renderer.info.render.calls,triangles:v.renderer.info.render.triangles,
   ...
   await page.screenshot({path:`output/playwright/attraction-${piece.kind}-${mobile?'mobile':'desktop'}.png`});
