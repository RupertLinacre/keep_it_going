...
export function snapshotRide(game: Mini, seq: number): RideState {
  ...
  return {
    ...
    parcels: game.carriages.parcels.map(p => ({ id: entityId(p, "parcel"), velocity: p.velocity.toArray(), spin: p.angularVelocity.toArray(), position: p.position.toArray(), rotation: p.rotation.toArray() })),
